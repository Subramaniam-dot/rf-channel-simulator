...
class QAM64(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        gr.top_block.__init__(self, "64 QAM", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("64 QAM")
        ...
        self.filename = filename = "QAM_64"
        self.QAM_64 = QAM_64 = digital.constellation_calcdist([-7+7j, -5+7j, -3+7j, -1+7j,  1+7j,  3+7j,  5+7j,  7+7j, -7+5j, -5+5j, -3+5j, -1+5j,  1+5j,  3+5j,  5+5j,  7+5j, -7+3j, -5+3j, -3+3j, -1+3j,  1+3j,  3+3j,  5+3j,  7+3j, -7+1j, -5+1j, -3+1j, -1+1j,  1+1j,  3+1j,  5+1j,  7+1j, -7-1j, -5-1j, -3-1j, -1-1j,  1-1j,  3-1j,  5-1j,  7-1j, -7-3j, -5-3j, -3-3j, -1-3j,  1-3j,  3-3j,  5-3j,  7-3j, -7-5j, -5-5j, -3-5j, -1-5j,  1-5j,  3-5j,  5-5j,  7-5j, -7-7j, -5-7j, -3-7j, -1-7j,  1-7j,  3-7j,  5-7j,  7-7j], [0, 1, 3, 2, 6, 7, 5, 4, 8, 9, 11, 10, 14, 15, 13, 12, 24, 25, 27, 26, 30, 31, 29, 28, 16, 17, 19, 18, 22, 23, 21, 20, 48, 49, 51, 50, 54, 55, 53, 52, 56, 57, 59, 58, 62, 63, 61, 60, 40, 41, 43, 42, 46, 47, 45, 44, 32, 33, 35, 34, 38, 39, 37, 36],
        4, 1, digital.constellation.AMPLITUDE_NORMALIZATION).base()
        self.QAM_64.set_npwr(0.000)
        self.M = M = 64
        ...
        self.top_layout.addWidget(_id_write_button_0_0_toggle_button)
        self.epy_block_1 = epy_block_1.blk(filename="output", num_samples=2048, modulation_scheme="64QAM", snr=SNR, freq_offset=freq_offset, max_files=100)
        ...
        self.msg_connect((self.id_write_button_0_0, 'pressed'), (self.epy_block_1, 'enable_write'))
    ...
    def set_signal_voltage(self, signal_voltage):
        self.signal_voltage = signal_voltage
        self.set_noise_volt((lambda x: self.signal_voltage / __import__('math').pow(10, x / 20))(self.SNR)
    ...
    def set_freq_offset(self, freq_offset):
        ...
        self.id_write_button_0_0.set_new_msg(self.freq_offset)
